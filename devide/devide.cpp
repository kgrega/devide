...
void DevIde::commands(QString appname, QString testinput, QString testoutput){
    ...
    while(!outputsream.atEnd()){
      ...
      ui->compilerouttext->append("Program áll adott eredmény:" + resultstr);
      ui->compilerouttext->append(resultstr + result.setNum(compare));            
      if(compare==1){
        step++;}}
    resultvalue=(step/i);

    ui->compilerouttext->append("Eredmény sikeressége:" + QString::number(step) + "/" + QString::number(i));
...
{
    if(ui->inputstr->text()=="" && ui->outputstr->text()==""){ui->compilerouttext->append("Üres kimenet/bemeneti fájl!");
    }else{
    QListWidgetItem *list;
    list = ui->projectree->currentItem();
    if(list!=nullptr){
...
void DevIde::newproject(){
    ...
    //projecttype.append(data);
   /*    if(projecttype=="C++"){
            ui->devareatext->append("#include <stdio.h>\n");
            ui->devareatext->append("\n\n");
            ui->devareatext->append("int main(){\n printf(Hello World!);\n return 0;} \n");*/
       // }
...
void::DevIde::Debprojectopen(QString projectfilename){
    ...
    foreach (QString data, debfilelist) {
        projectfile.setFileName(projectpath.path() + "/" + data);
        QTextStream openfilestream(&projectfile);
        projectfile.open(QIODevice :: ReadOnly | QIODevice :: Text);
        ui->devareatext->append(openfilestream.readAll());
        ui->projectree->addItem(projectfile.fileName());
        tmpfile.append(projectfile.fileName()+".tmp");
        projectfile.copy(projectfile.fileName()+".tmp");
        projectfile.close();}
    openprojecttype="debpackage";
}
