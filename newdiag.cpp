    ...
    ui(new Ui::Newdiag)
{
    ...
    if(QFile::exists(QDir::currentPath() + "/profiles/gui.opt")){
        ...
        ui->projecttype->setStyleSheet(listcolor);
        ui->cancel_button->setStyleSheet(buttoncheckboxcolor);
        ui->createprojectbutton->setStyleSheet(buttoncheckboxcolor);
        ui->project_name->setStyleSheet(buttoncheckboxcolor);
        ui->project_path->setStyleSheet(textlinecolor);
        ui->project_path_set->setStyleSheet(buttoncheckboxcolor);}
         this->setWindowTitle(tr("Új projekt létrehozzása"));
    //
}
...
void Newdiag::on_createprojectbutton_pressed()
{
    ...
    projectfilestr << "FILE=" + ui->project_name->text() + ".cpp\n";

    projectfile.close();
    projectfile.setFileName(ui->project_path->text() + ui->project_name->text() + "/" + ui->project_name->text() + ".cpp");
    projectfile.open(QIODevice:: WriteOnly | QIODevice:: Text);
...
void Newdiag::on_projecttype_currentTextChanged(const QString &newprojecttype)
{
    ...
    //////////////2024_03_10
    if(newprojecttype=="Add file" && QFile::exists(QDir::currentPath() + "/gitproject")){
        projectfile.setFileName(QDir::currentPath() + "/gitproject");
        projectfile.open(QIODevice:: ReadOnly | QIODevice:: Text);
        path=projectfilestr.readLine();
        ui->project_path->setText(path);
        ui->project_path->setDisabled(1);}
    if(newprojecttype=="Add file"){
        ui->project_name_label->setText("Fájl név");}
    else{ui->project_name_label->setText("Project neve");}
